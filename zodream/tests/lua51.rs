...
#[test]
fn test_duble() {
    let buffer = std::fs::read("tests/lua51/concat-duble.luac");
    let parsed = parse(&buffer.unwrap()).unwrap();
...
#[test]
fn test_int() {
    let parsed = parse(&std::fs::read("tests/lua51/concat-int.luac").unwrap()).unwrap();
    println!("{:#?}", parsed.main_chunk.constants);
