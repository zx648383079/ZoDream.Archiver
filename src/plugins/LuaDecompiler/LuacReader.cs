...
namespace ZoDream.LuaDecompiler
{
    public partial class LuacReader: ILanguageReader
    {
        ...
        private GlobalExpression CreateBuilder(LuaBytecode data)
        {
            ...
            Translate(builder, data.MainChunk);
            return builder;
        }

        private void Translate(GlobalExpression builder, LuaChunk chunk)
        {
            var index = 0;
            foreach (var item in chunk.OpcodeItems)
            {
                Translate(builder, chunk, ++index, (OperandCode)item);
            }
        }

        private void Translate(GlobalExpression builder, LuaChunk chunk, int index, OperandCode code)
        {
