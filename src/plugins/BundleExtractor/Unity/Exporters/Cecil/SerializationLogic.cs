    ...
    internal static class SerializationLogic
    {
        ...
        public static bool HasSerializeFieldAttribute(FieldDefinition field)
        {
            ...
            foreach (var attribute in FieldAttributes(field))
                if (EngineTypePredicates.IsSerializeFieldAttribute(attribute))
                    return true;
            return false;
        }
        ...
        public static bool IsNonSerialized(TypeReference typeDeclaration)
        {
            ...
            if (typeDeclaration.IsArray)
                return true;
            if (typeDeclaration.FullName == EngineTypePredicates.MonoBehaviour)
                return true;
            if (typeDeclaration.FullName == EngineTypePredicates.ScriptableObject)
                return true;
