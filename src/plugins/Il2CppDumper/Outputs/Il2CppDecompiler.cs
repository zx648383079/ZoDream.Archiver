            ...
            foreach (var imageDef in metadata.imageDefs)
            {
                try
                {
                    ...
                    for (int typeDefIndex = imageDef.typeStart; typeDefIndex < typeEnd; typeDefIndex++)
                    {
                        ...
                        writer.WriteLine()
                            .WriteLine($"// Namespace: {metadata.GetStringFromIndex(typeDef.namespaceIndex)}");
                        ...
                        {
                            writer.WriteLine().WriteLine("\t// Properties");
                        ...
                        {
                            writer.WriteLine().WriteLine("\t// Methods");
                            ...
                            for (var i = typeDef.methodStart; i < methodEnd; ++i)
                            {
                                ...
                                if (config.DumpMethodOffset)
                                {
                                    ...
                                    if (!isAbstract && methodPointer > 0)
                                    {
                                        ...
                                        writer.WriteFormat("\t// RVA: 0x{0:X} Offset: 0x{1:X} VA: 0x{2:X}", fixedMethodPointer, il2Cpp.MapVATR(methodPointer), methodPointer);
                                    }
                                    else
                                    {
                                    ...
                                    {
                                        writer.WriteFormat(" Slot: {0}", methodDef.slot);
                                    }
                                    writer.WriteLine();
                                }
                                writer.Write("\t");
                                ...
                                {
                                    writer.WriteLine("\t/* GenericInstMethod :");
                                    ...
                                    foreach (var group in groups)
                                    {
                                        ...
                                        if (genericMethodPointer > 0)
                                        {
                                            ...
                                            writer.WriteLine($"\t|-RVA: 0x{fixedPointer:X} Offset: 0x{il2Cpp.MapVATR(genericMethodPointer):X} VA: 0x{genericMethodPointer:X}");
                                        }
                                        else
                                        {
                                            writer.WriteLine("\t|-RVA: -1 Offset: -1");
                                        }
                                        foreach (var methodSpec in group)
                                        {
                                            (var methodSpecTypeName, var methodSpecMethodName) = executor.GetMethodSpecName(methodSpec);
                                            writer.WriteLine($"\t|-{methodSpecTypeName}.{methodSpecMethodName}");
                                        }
                                    }
                                    writer.WriteLine("\t*/");
                                }
                ...
                {
                    Debug.WriteLine("ERROR: Some errors in dumping");
                    writer.Write("/*");
                    writer.Write(e);
                    writer.WriteLine("*/");
                    writer.WriteLine("}");
