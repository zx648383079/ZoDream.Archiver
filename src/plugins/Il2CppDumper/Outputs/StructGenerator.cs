...
namespace Il2CppDumper
{
    public class StructGenerator
    {
        ...
        private readonly Dictionary<Il2CppTypeDefinition, string> typeDefImageNames = [];
        private readonly HashSet<string> structNameHashSet = new(StringComparer.Ordinal);
        private readonly List<StructInfo> structInfoList = [];
        private readonly Dictionary<string, StructInfo> structInfoWithStructName = [];
        private readonly HashSet<StructInfo> structCache = [];
        private readonly Dictionary<Il2CppTypeDefinition, string> structNameDic = [];
        private readonly Dictionary<ulong, string> genericClassStructNameDic = [];
        private readonly Dictionary<string, Il2CppType> nameGenericClassDic = [];
        private readonly List<ulong> genericClassList = [];
        private readonly StringBuilder arrayClassHeader = new();
        private readonly StringBuilder methodInfoHeader = new();
        private static readonly HashSet<ulong> methodInfoCache = [];
        ...
        public void WriteScript(string outputDir)
        {
            ...
            using (var fs = File.Create(Path.Combine(outputDir, "stringliteral.json")))
            {
                JsonSerializer.Serialize(fs, stringLiterals, jsonOptions);
            }
            //写入文件
            using (var fs = File.Create(Path.Combine(outputDir, "script.json")))
            {
                JsonSerializer.Serialize(fs, json, jsonOptions);
            }
            //il2cpp.h
            ...
            using var sb = new CodeWriter(File.Create(Path.Combine(outputDir, "il2cpp.h")));
            sb.Write(HeaderConstants.GenericHeader);
            switch (il2Cpp.Version)
            {
                case 22:
                    sb.Write(HeaderConstants.HeaderV22);
                    break;
                case 23:
                case 24:
                    sb.Write(HeaderConstants.HeaderV240);
                    break;
                case 24.1:
                    sb.Write(HeaderConstants.HeaderV241);
                    break;
                case 24.2:
                case 24.3:
                case 24.4:
                case 24.5:
                    sb.Write(HeaderConstants.HeaderV242);
                    break;
                case 27:
                case 27.1:
                case 27.2:
                    sb.Write(HeaderConstants.HeaderV27);
                    break;
                case 29:
                case 29.1:
                case 31:
                    sb.Write(HeaderConstants.HeaderV29);
                    break;
                default:
                    Debug.WriteLine($"WARNING: This il2cpp version [{il2Cpp.Version}] does not support generating .h files");
                    return;
            }
            ...
            foreach (var info in structInfoList)
            {
                RecursionStructInfo(sb, info);
            }
            sb.Write(arrayClassHeader);
            sb.Write(methodInfoHeader);
        }
        ...
        private void RecursionStructInfo(ICodeWriter writer, StructInfo info)
        {
            ...
            if (info.Parent != null)
            {
                ...
                RecursionStructInfo(writer, structInfoWithStructName[parentStructName]);
            ...
            foreach (var field in info.Fields)
            {
                if (field.IsValueType)
                {
                    ...
                    RecursionStructInfo(writer, fieldInfo);
                }
