        ...
        */
        ...
        protected override void Load()
        {
            ...
            if (!IsDumped)
            {
                ...
                if (CheckProtection())
                {
                    Debug.WriteLine("ERROR: This file may be protected.");
                }
            }
        }
        ...
        public override bool Search()
        {
            ...
            if (resultList.Count == 1)
            {
                ...
                Debug.WriteLine("CodeRegistration : {0:x}", codeRegistration);
                Debug.WriteLine("MetadataRegistration : {0:x}", metadataRegistration);
        ...
        public override bool SymbolSearch()
        {
            ...
            {
                Debug.WriteLine("Detected Symbol !");
                Debug.WriteLine("CodeRegistration : {0:x}", codeRegistration);
                Debug.WriteLine("MetadataRegistration : {0:x}", metadataRegistration);
                Init(codeRegistration, metadataRegistration);
                return true;
            }
            Debug.WriteLine("ERROR: No symbol is detected");
            return false;
        }
        ...
        {
            Debug.WriteLine("Applying relocations...");
            try
            {
        ...
        private bool CheckProtection()
        {
            try
            {
                ...
                {
                    Debug.WriteLine("WARNING: find .init_proc");
                    return true;
                }
                //JNI_OnLoad
                var dynstrOffset = MapVATR(dynamicSection.First(x => x.d_tag == DT_STRTAB).d_un);
                foreach (var symbol in symbolTable)
                {
                    ...
                    {
                        case "JNI_OnLoad":
                            Debug.WriteLine("WARNING: find JNI_OnLoad");
                            return true;
                    }
                }
                if (sectionTable != null && sectionTable.Any(x => x.sh_type == SHT_LOUSER))
                {
                    Debug.WriteLine("WARNING: find SHT_LOUSER section");
                    return true;
                }
            }
            catch
