...
{
    public class EACRGUnsigned : IBufferDecoder
    {
        ...
        public int Decode(ReadOnlySpan<byte> data, int width, int height, Span<byte> output)
        {
            ...
            for (int t = 0; t < bch; t++)
            {
                ...
                {
                    DecodeEacUnsignedBlock(data.Slice(inputOffset + 0, 8), buf, 2);
                    DecodeEacUnsignedBlock(data.Slice(inputOffset + 8, 8), buf, 1);
