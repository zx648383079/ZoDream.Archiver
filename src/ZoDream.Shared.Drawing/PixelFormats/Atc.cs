...
namespace ZoDream.Shared.Drawing
{
    public class ATC(BitmapFormat format) : IBufferDecoder
    {
        ...
        {
            if (format is BitmapFormat.ATC_RGB4)
            {
                return DecodeAtcRgb4(data, width, height, output);
            }
            if (format is BitmapFormat.ATC_RGB8)
            {
        ...
        private static int DecodeAtcRgb4(ReadOnlySpan<byte> input, int width, int height, Span<byte> output)
        {
            int bcw = (width + 3) / 4;
            int bch = (height + 3) / 4;
        ...
        private static int DecodeAtcRgba8(ReadOnlySpan<byte> input, int width, int height, Span<byte> output)
        {
            int bcw = (width + 3) / 4;
            int bch = (height + 3) / 4;
            int clen_last = (width + 3) % 4 + 1;
