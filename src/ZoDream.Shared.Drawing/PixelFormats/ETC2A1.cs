...
{
    public class ETC2A1 : IBufferDecoder
    {
        ...
        public int Decode(ReadOnlySpan<byte> data, int width, int height, Span<byte> output)
        {
            ...
            for (int t = 0; t < bch; t++)
            {
                ...
                {
                    DecodeEtc2a1Block(data.Slice(inputOffset, 8), buf);
        ...
        private static void DecodeEtc2a1Block(ReadOnlySpan<byte> input, Span<byte> output)
        {
            if ((input[3] & 2) != 0)
            {
                // Opaque
                ETC2.DecodeEtc2Block(input, output);
            }
            else
            {
